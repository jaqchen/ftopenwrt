...
function make_boottone_fex()
{
	...
	if [ -e $BOOTTONE_FILE_PATH ];then
		...
		echo "${PACK_TOPDIR}/out/host/bin/make_ext4fs -l 512k -b 1024 -m 0 -J ${ROOT_DIR}/image/boottone.fex $BOOTTONE_FILE_PATH"
...
function do_prepare()
{
	...
	# amend env copy
	mv -v ${ROOT_DIR}/image/env-${ENV_SUFFIX}.cfg ${ROOT_DIR}/image/env.cfg 2> /dev/null
	...
	for file in ${boot_resource_list[@]} ; do
		cp -v -rf `echo $file | awk -F: '{print $1}'` \
	...
	cp -v ${ROOT_DIR}/image/env.cfg  ${ROOT_DIR}/image/env_bk.cfg
	...
	if [ -e ${ROOT_DIR}/image/sys_partition_nor.fex ];then
		echo "make_user_res ${ROOT_DIR}/image/sys_partition_nor.fex"
		make_user_res ${ROOT_DIR}/image/sys_partition_nor.fex

		echo "make_app_res ${ROOT_DIR}/image/sys_partition_nor.fex"
		make_app_res ${ROOT_DIR}/image/sys_partition_nor.fex

		echo "make_data_res ${ROOT_DIR}/image/sys_partition_nor.fex"
		make_data_res ${ROOT_DIR}/image/sys_partition_nor.fex

		echo "update_mbr_to_sys_config ${ROOT_DIR}/image/sys_partition_nor.fex"
		update_mbr_to_sys_config ${ROOT_DIR}/image/sys_partition_nor.fex
	else
		echo "make_user_res ${ROOT_DIR}/image/sys_partition.fex"
		make_user_res ${ROOT_DIR}/image/sys_partition.fex

		echo "make_app_res ${ROOT_DIR}/image/sys_partition.fex"
		make_app_res ${ROOT_DIR}/image/sys_partition.fex

		echo "make_data_res ${ROOT_DIR}/image/sys_partition.fex"
		make_data_res ${ROOT_DIR}/image/sys_partition.fex

		echo "update_mbr_to_sys_config ${ROOT_DIR}/image/sys_partition.fex"
...
function do_finish()
{
	...
	if [ -f sys_partition_nor.bin ]; then
		echo "update_mbr sys_partition_nor.bin 1 sunxi_mbr_nor.fex"
		...
		echo "mbr2env sunxi_mbr_nor.fex env_nor.fex"
		...
		if [ "x${PACK_KERN}" = "xlinux-3.4" ] ; then
			...
			echo "merge_full_img --out full_img.fex --boot0 boot0_spinor.fex --boot1 ${BOOT1_FILE} --mbr ${mbr_file} --logic_start ${LOGIC_START} --uboot_start ${UBOOT_START} --partition sys_partition_nor.bin"
	...
	if [ -f sys_partition_for_dragon.fex ]; then
		echo "do_dragon image.cfg sys_partition_for_dragon.fex"
		do_dragon image.cfg sys_partition_for_dragon.fex
	else
		echo "do_dragon image.cfg"
		do_dragon image.cfg
	fi

	if [ "x${PACK_PROGRAMMER}" = "xprogrammer" ]; then
		echo "waiting to ceate programmer img..."
		echo "img_to_programmer ${IMG_PROGRAMMER_NAME} ../${IMG_NAME}"
...
{
	printf "packing for tina linux in [${PWD}]\n"
	...
	if [ -f ${ROOT_DIR}/boot_initramfs.img ]; then
		ln -s -v ${ROOT_DIR}/boot_initramfs.img        boot.fex
	else
		ln -s -v ${ROOT_DIR}/boot.img        boot.fex
	fi
	# sys_partition_nor.fex in longan may use kernel.fex
	ln -s -v boot.fex        kernel.fex

	if [ -f ${ROOT_DIR}/${PACK_BOARD}-Image.gz ] ; then
		cp -v ${ROOT_DIR}/${PACK_BOARD}-Image.gz Image.gz
	fi

	if [ -f Image.gz -a -f fit-image.its ] ; then
		echo "mkimage -f fit-image.its fit-image.fex"
	...
	if [ "x${PACK_SIG}" = "xsecure" ] ; then
		...
		ln -s -v ${ROOT_DIR}/rootfs.img     rootfs.fex
		[ -f dtbo.img ] &&
			ln -s -v dtbo.img        dtbo.fex
	fi
	...
	ln -s -v rootfs.fex rootfs_nor.fex
	ln -s -v rootfs.fex rootfs-ubifs.fex

	if [ -f ${ROOT_DIR}/usr.img ]; then
		ln -s -v ${ROOT_DIR}/usr.img    usr.fex
	fi

	if [ -f ${ROOT_DIR}/boot_initramfs_recovery.img ]; then
		ln -s -v ${ROOT_DIR}/boot_initramfs_recovery.img recovery.fex
	else
		touch recovery.fex
		echo "recovery part not used!" | tee recovery.fex
	fi
	...
	if [ "x${PACK_SIG}" = "xsecure" ] ; then
		...
		grep "CONFIG_TARGET_ROOTFS_SQUASHFS=y" ${PACK_TOPDIR}/.config > /dev/null && \
			grep "CONFIG_USE_UBOOT_VERIFY_SQUASHFS=y" ${PACK_TOPDIR}/.config > /dev/null
		if [ $? -eq 0 ]; then
			echo "update_squashfs ${ROOT_DIR}/image/rootfs.fex ${ROOT_DIR}/image/toc1/cert/rootfs.der"
...
function do_pack_partition_file()
{
	...
	echo "tar -zcv -h -f img_${tarimg_name}.tar.gz $tar_file readme > /dev/null"
...
if [ "x${PACK_TAR_IMAGE}" = "xtar_image" ]; then
	echo "do_pack_partition_file ${ROOT_DIR}/image/sys_partition.fex"
	do_pack_partition_file ${ROOT_DIR}/image/sys_partition.fex
fi

if [ -e ${PACK_TOPDIR}/scripts/.hooks/post-pack ] ; then
	echo "source ${PACK_TOPDIR}/scripts/.hooks/post-pack"
